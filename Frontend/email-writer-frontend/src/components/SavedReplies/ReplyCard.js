...
const ReplyCard = ({ reply, onToggleFavorite, onDelete }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // Error toast is shown by the parent (SavedReplies.handleDeleteReply)
    } finally {
      // Moved from try to finally — modal now closes whether delete
      // succeeds or fails, preventing the modal + error toast overlap.
            ...
            >
              <div className="full-reply-modal">
                ...
                <div className="full-reply-actions">
                  <button onClick={handleCopyReply} className="btn btn-outline">
                    <i className="fas fa-copy"></i>
                    Copy to Clipboard
                  </button>
